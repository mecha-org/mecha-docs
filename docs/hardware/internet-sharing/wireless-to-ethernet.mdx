...
The purpose of this setup is to share a wireless internet connection over an Ethernet connection, enabling other devices to access the internet via the Ethernet network created on Comet.
...
### 1. Connect Comet to a Device with an Ethernet Port

Physically connect the Comet to another device (e.g., a laptop or desktop) using an Ethernet cable. Ensure the Ethernet port is functional on both devices.
