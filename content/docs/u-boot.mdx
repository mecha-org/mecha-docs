...
- **Hardware Variants**: Automatically handling configurations for both the 4GB and 8GB RAM models.
...
**For the 4GB RAM Model:**
```bash
make mecha-comet-imx8mp-4gb_defconfig
make -j$(nproc)
```

**For the 8GB RAM Model:**
```bash
make mecha-comet-imx8mp-8gb_defconfig
make -j$(nproc)
```
...
<Callout type="info">
The Mecha Comet U-Boot repository uses [GitHub Actions](https://github.com/mecha-org/u-boot/actions) to automatically build and release boot images for both 4GB and 8GB variants. You can find pre-built `flash.bin` binaries in the repository's actions tab.
</Callout>
